...
class DeviceManager:
    ...
    def run_ping_test_all_devices(self):
        """Runs the ping test for all devices using the default source and target IP."""
    ...
    def configure_port_security(self):
        if 'port_security' in self.device_info:
            for port_sec in self.device_info['port_security']:
                ...
                print(f"Configuring Port Security on {interface} for {self.device_name}...")
    ...
    def shutdown_unused_ports(self):
        if self.device_info.get('shutdown_unused_ports', False):
            print(f"Shutting down unused ports on {self.device_name}...")
            unused_ports = ShutdownUnusedPortsConfig(self.ssh_manager)
    ...
    def configure_hsrp(self):
        if self.device_name in self.hsrp_devices:
            ...
            hsrp_manager = HSRPConfig(self.ssh_manager, hsrp_config['interface'], hsrp_config['group'],
                                      hsrp_config['real_ip'], hsrp_config['standby_ip'], hsrp_config['priority'])
    ...
    def configure_dhcp(self):
        if 'dhcp' in self.device_info:
            ...
            dhcp_config = DHCPConfig(self.ssh_manager, dhcp_info['pool_name'], dhcp_info['network'],
                                     dhcp_info['mask'], dhcp_info['default_router'], dhcp_info['dns_server'],
                                     dhcp_info['excluded_addresses'])
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            break  # Iese din bucla principală și din program
